...
const sectors = [
  {
    ...
    statColor:   "text-teal",
    gradientStyle: {
      background: "linear-gradient(135deg, rgba(0,200,176,0.10) 0%, rgba(0,200,176,0.03) 60%, transparent 100%)",
    },
    ...
    image:       "/images/gallery/sisakhola-river-01.png",
    hasImage:    true,
  },
  {
    id:          "solar",
    ...
    statColor:   "text-gold",
    gradientStyle: {
      background: "linear-gradient(135deg, rgba(240,192,64,0.10) 0%, rgba(240,192,64,0.03) 60%, transparent 100%)",
    },
  ...
  },
  {
    ...
    gradientStyle: {
      background: "linear-gradient(135deg, rgba(0,212,106,0.10) 0%, rgba(0,212,106,0.03) 60%, transparent 100%)",
    },
    ...
    image:       "/images/gallery/rice-mill-exterior-trucks.png",
    hasImage:    true,
  },
];
  ...
  return (
    <Section id="projects-overview">
      <Container>
        ...
        >
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <div>
              ...
              <p className="mt-3 text-foreground-muted max-w-lg leading-relaxed">
                Integrated development across clean energy and agro-industry — building Nepal&apos;s
                economic and environmental future from the ground up.
              </p>
            </div>
            ...
            return (
              ...
              >
                ...
                <div className="relative z-10 p-6 flex flex-col h-full">
                  {/* Top row: icon + sector label + status */}
                  <div className="flex items-start justify-between mb-5">
                    ...
                    <span className={`text-[10px] font-semibold px-2.5 py-1 rounded-full border ${sector.statusColor}`}>
                      {sector.status}
                    </span>
                  </div>

                  {/* Sector label */}
                  <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-foreground-subtle mb-1">
                    {sector.sector}
                  </p>
                  <h3 className="text-xl font-display font-bold text-foreground mb-1 tracking-tight">
                    {sector.label}
                  </h3>

                  {/* Key stat */}
                  <div className="flex items-baseline gap-1.5 mb-4">
                    <span className={`text-3xl font-display font-bold leading-none ${sector.statColor}`}>
                      {sector.stat}
                    </span>
                  ...
                  <Link
                    href={sector.href}
                    className={`inline-flex items-center gap-1.5 text-sm font-medium transition-colors ${sector.iconColor} hover:opacity-80`}
                  >
        ...
        {/* Bottom total stats strip */}
        <motion.div
          initial={{ opacity: 0, y: 16 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true, margin: "-40px" }}
          transition={{ duration: 0.6, delay: 0.3 }}
          className="mt-8 rounded-2xl border border-border bg-surface-raised p-6"
        >
          <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
            <div className="grid grid-cols-3 gap-8 sm:gap-16">
              {[
                { value: "14.9 MW", label: "Total Energy Pipeline" },
                { value: "2",       label: "Active Energy Projects" },
                { value: "3",       label: "Business Sectors" },
              ].map(({ value, label }) => (
                <div key={label} className="text-center sm:text-left">
                  <p className="text-xl md:text-2xl font-display font-bold text-gradient leading-none mb-0.5">
                    {value}
                  </p>
                  <p className="text-[10px] text-foreground-subtle uppercase tracking-wide">{label}</p>
                </div>
              ))}
            </div>
