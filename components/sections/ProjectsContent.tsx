...
import { Droplets, Sun, Sprout, Globe, MapPin, CheckCircle, Mountain, ArrowUpRight } from "lucide-react";
...
const categoryAccent: Record<string, { icon: string; border: string; gradient: string }> = {
  "Hydropower":   { icon: "text-teal bg-teal/10 border-teal/20",          border: "hover:border-teal/30",    gradient: "from-teal/5" },
  "Solar Energy": { icon: "text-gold bg-gold/10 border-gold/20",          border: "hover:border-gold/30",    gradient: "from-gold/5" },
  "Agriculture":  { icon: "text-primary bg-primary/10 border-primary/20", border: "hover:border-primary/30", gradient: "from-primary/5" },
  "Tourism":      { icon: "text-earth bg-earth/10 border-earth/20",       border: "hover:border-earth/30",   gradient: "from-earth/5" },
};
              ...
              >
                ...
                >
                  {filtered.map((p) => {
                    ...
                    const capacityStr     = p.capacity ? `${p.capacity.value} ${p.capacity.unit}` : "—";
                    ...
                    return (
                      <motion.div
                        ...
                        className={cn(
                          "group rounded-2xl bg-card border border-border overflow-hidden transition-colors duration-300",
                          accent.border
                        )}
                      >
                        {/* Cover image */}
                        {p.coverImage && (
                          <div className="relative h-44 overflow-hidden">
                            ...
                            />
                            <div className={cn("absolute inset-0 bg-linear-to-br opacity-30", accent.gradient)} />
                          </div>
                        )}
                        ...
                        <div className="p-6">
                          ...
                          <div className="flex items-start justify-between gap-4 mb-4">
                            <div className="flex items-start gap-3">
                              ...
                              <div>
                                <h3 className="font-semibold text-foreground leading-snug text-base mb-1 group-hover:text-primary transition-colors">
                                  {p.name}
                                </h3>
                          ...
                          {/* Description */}
                          <p className="text-sm text-foreground-muted leading-relaxed mb-4 line-clamp-3">{p.description}</p>
